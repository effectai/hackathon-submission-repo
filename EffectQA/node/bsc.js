...
const main = async () => {
  ...
  if (process.argv.length <= 2) {
    ...
    await askQuestion(
      "Press ENTER to continue after you have recieved funds from the faucet bot....",
      (answer) => {
        console.log(
          "Thanks! Now let's continue, making the campaign and batches."
        );
        rl.close();
      }
    );
  ...
  const campaignToIpfs = {
    ...
    example_task: {
      image_url:
        "https://ipfs.effect.ai/ipfs/bafkreidrxwhqsxa22uyjamz7qq3lh7pv2eg3ykodju6n7cgprmjpal2oga",
    },
    version: 1,
    reward: 1,
  };
